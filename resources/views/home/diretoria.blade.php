                            ...
                            @foreach($diretorias as $key => $diretoria)
                                <div class="mb-3">
                                    <a class="badge badge-primary text-uppercase font-weight-semi-bold p-2 mr-2"
                                        href="{{route('home.home')}}">Diretoria</a>
                                    <a class="text-body">{{$diretoria->created_at}}</a>
                                </div>
                                <h1 class="mb-3 text-secondary text-uppercase font-weight-bold">
                                    {!! substr(strip_tags($diretoria->titulo), 0, 50) !!}
                                    <!-- Exibe os primeiros 50 caracteres do título -->
                                    @if(strlen(strip_tags($diretoria->titulo)) > 50)
                                        [...]
                                    @endif
                                </h1>
                                <p>{!! $diretoria->conteudo!!}</p>
                            @endforeach
                        </div>
                    </div>
