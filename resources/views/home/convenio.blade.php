                        ...
                        <div class="bg-white border border-top-0 p-4">
                            @foreach($convenios as $key => $convenio)
                                <div class="mb-3">
                                    ...
                                    <a class="text-body">{{$convenio->created_at}}</a>
                                </div>
                                <h1 class="mb-3 text-secondary text-uppercase font-weight-bold">
                                {!! substr(strip_tags($convenio->titulo), 0, 50) !!} <!-- Exibe os primeiros 100 caracteres do conteúdo -->
                                    @if(strlen(strip_tags($convenio->titulo)) > 50)
                                        [...]
                                    @endif
                                </h1>
                                <p>
