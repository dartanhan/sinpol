                ...
                <div class="card shadow-sm border-0">
                    <div class="card-header bg-primary text-white py-3 d-flex align-items-center justify-content-between">
                        <h5 class="card-title m-0 p-0 text-white text-uppercase fw-bold">
                            <i class="bi bi-plus-circle me-2"></i>Criar Notícia
                        </h5>
                    ...
                    <div class="card-body p-4">
                        <form method="POST" action="{{route('noticia.store')}}" name="noticiaForm" id="noticiaForm" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="idImagemDestaque" id="idImagemDestaque" value="{{ old('idImagemDestaque') }}"/>
                            
                            <div class="row g-3">
                                <div class="col-md-8 text-start">
                                    <label for="titulo" class="form-label fw-bold">Título da Notícia</label>
                                    <input type="text" name="titulo" id="titulo" class="form-control" placeholder="Título impactante para a notícia" maxlength="60" required value="{{ old('titulo') }}">
                                    <small class="text-muted">Máximo 60 caracteres.</small>
                                </div>
                                <div class="col-md-4 text-start">
                                    <label for="slug" class="form-label fw-bold">Slug</label>
                                    <input type="text" name="slug" id="slug" readonly class="form-control bg-light" placeholder="Gerado automaticamente" value="{{ old('slug') }}">
                                </div>
                                
                                <div class="col-md-12 text-start">
                                    <label for="subtitulo" class="form-label fw-bold">Subtítulo (Resumo)</label>
                                    <textarea name="subtitulo" id="subtitulo" class="form-control" rows="2" placeholder="Um breve resumo que aparecerá no slider e listagens" maxlength="250">{{ old('subtitulo') }}</textarea>
                                </div>
                                ...
                                <div class="col-md-4 text-start">
                                    ...
                                    <select name="status" id="status" class="form-select" required>
                                        <option value="1" {{ old('status', '1') == '1' ? 'selected' : '' }}>ATIVO / PUBLICADO</option>
                                ...
                                <div class="col-12" id="editorContainer" style="display: {{ old('idImagemDestaque') ? 'block' : 'none' }};">
                            ...
                            <div class="text-end mt-4">
                                ...
                                <button type="submit" class="btn btn-primary px-5 shadow-sm text-uppercase fw-bold">Salvar Notícia</button>
                            </div>
                        </form>
                    </div>
                </div>
