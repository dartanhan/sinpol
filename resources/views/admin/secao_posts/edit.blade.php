                ...
                <div class="card shadow-sm border-0">
                    <div class="card-header bg-primary text-white py-3 d-flex align-items-center justify-content-between">
                        <h5 class="card-title m-0 p-0 text-white text-uppercase fw-bold">
                            <i class="bi bi-pencil-square me-2"></i>Editar Entrada em {{ $tituloDaTela }}
                        </h5>
                    ...
                    <div class="card-body p-4">
                        <form method="POST" action="{{route($tipo.'.update', $post->id)}}" name="secaoForm" id="secaoForm" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="row g-3">
                                <div class="col-md-12">
                                    <label for="titulo" class="form-label fw-bold">Título do Post</label>
                                    <input type="text" name="titulo" id="titulo" class="form-control" placeholder="Título" maxlength="150" required value="{{ old('titulo', $post->titulo) }}">
                                ...
                                <div class="col-md-12 mt-3">
                                    ...
                                    <select name="status" id="status" class="form-select">
                                        <option value="1" {{ old('status', $post->status) == 1 ? 'selected' : '' }}>ATIVO / PUBLICADO</option>
                                        <option value="0" {{ old('status', $post->status) == 0 ? 'selected' : '' }}>INATIVO / RASCUNHO</option>
                                    </select>
                                </div>
                                ...
                                <div class="col-12 mt-4">
                                    <div class="card border-0 shadow-sm">
                                        ...
                                        <div class="card-body p-0">
                                            <textarea class="tinymce_editor" name="tinymce_editor" id="tinymce_editor">{{ old('tinymce_editor', $post->conteudo) }}</textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
